    ...
    <div className="min-h-screen bg-[#0a0e27] text-white" onClick={() => set({ isRegionDropdownOpen: false, isProfileDropdownOpen: false } as any)}>
      ...
      {isVerificationOpen && (
        <EmailVerificationModal
          ...
          setEnteredCode={(v: string) => set({ enteredCode: v })}
      ...
      {isPremiumOpen && (
        <PremiumModal
          ...
          setIsPostBackgroundPickerOpen={(v: boolean) => set({ isPostBackgroundPickerOpen: v })}
          postImage={postImage}
          setPostImage={(v: string) => set({ postImage: v })}
          tempPostImage={store.tempPostImage}
          setTempPostImage={(v: string) => set({ tempPostImage: v })}
          imageZoom={imageZoom}
          setImageZoom={(v: number) => set({ imageZoom: v })}
          imagePositionX={imagePositionX}
          setImagePositionX={(v: number) => set({ imagePositionX: v })}
          imagePositionY={imagePositionY}
          setImagePositionY={(v: number) => set({ imagePositionY: v })}
          hasPremium={hasPremium}
          setHasPremium={setHasPremium}
          postBorder={postBorder}
          setPostBorder={(v: string) => set({ postBorder: v })}
