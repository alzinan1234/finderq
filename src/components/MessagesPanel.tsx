    ...
    <>
      ...
      {openThreads.map((thread, index) => (
        <div
          ...
          style={{
            bottom: thread.minimized ? '0px' : '44px',
            left: `${24 + (index * 288) + 256}px`
          }}
        >
          <ChatThread
            thread={thread}
      ...
      <div
        className="fixed bottom-0 left-6 z-40 w-64"
        onMouseEnter={() => setIsHovered(true)}
        onMouseLeave={() => { setIsHovered(false); setFriendMenu(null); }}
      >
        {isHovered && (
          <div className="mb-2 bg-[#1a1d29] rounded-t-2xl border border-white/10 shadow-2xl overflow-hidden">
            <div className="flex border-b border-white/10">
              <button onClick={() => setActiveTab('messages')} className={`flex-1 flex flex-col items-center justify-center gap-0.5 py-2 text-[10px] font-medium transition-colors ${activeTab === 'messages' ? 'text-[#00d4ff] border-b-2 border-[#00d4ff]' : 'text-white/50 hover:text-white/80'}`}>
                <MessageCircle className="w-3.5 h-3.5" />
                Messages
              </button>
              <button onClick={() => setActiveTab('friends')} className={`flex-1 flex flex-col items-center justify-center gap-0.5 py-2 text-[10px] font-medium transition-colors ${activeTab === 'friends' ? 'text-[#00d4ff] border-b-2 border-[#00d4ff]' : 'text-white/50 hover:text-white/80'}`}>
                <Users className="w-3.5 h-3.5" />
                Friends
              </button>
              <button onClick={() => setActiveTab('requests')} className={`flex-1 flex flex-col items-center justify-center gap-0.5 py-2 text-[10px] font-medium transition-colors ${activeTab === 'requests' ? 'text-green-400 border-b-2 border-green-400' : 'text-white/50 hover:text-white/80'}`}>
              ...
              <button onClick={() => setActiveTab('blocked')} className={`flex-1 flex flex-col items-center justify-center gap-0.5 py-2 text-[10px] font-medium transition-colors ${activeTab === 'blocked' ? 'text-orange-400 border-b-2 border-orange-400' : 'text-white/50 hover:text-white/80'}`}>
                    ...
                    return (
                      <div key={friend.username} className="relative flex items-center gap-2 p-2 hover:bg-white/5 transition-colors group">
                        ...
                        <button
                          ...
                          className="flex-shrink-0 w-6 h-6 flex items-center justify-center rounded-lg opacity-0 group-hover:opacity-100 hover:bg-white/10 transition-all text-white/50 font-bold text-xs"
                        >
                          ···
                        </button>
...
function ChatThread({ thread, onClose, onSendMessage, onViewProfile, onToggleMinimize }: ChatThreadProps) {
  ...
  return (
    <div className={`bg-[#1a1d29] rounded-t-2xl border border-white/10 shadow-2xl flex flex-col w-72 ${minimized ? 'h-auto' : 'h-96'} transition-all duration-200`}>
      {/* Header */}
      ...
      {!minimized && (
        <>
          <div className="flex-1 overflow-y-auto p-3 space-y-2 bg-[#1a1d29]">
          ...
          <div className="p-2 border-t border-white/10 flex-shrink-0">
            <div className="flex gap-2">
              ...
              />
              <button onClick={handleSend} disabled={!message.trim()} className="px-2 bg-gradient-to-r from-[#00d4ff] to-[#00b8e6] text-white rounded-lg disabled:opacity-50">
